...
const copyText = () => {
  if (navigator.clipboard) {
    navigator.clipboard
      ...
      .catch((err) => alert("복사에 실패했습니다.", err.message));
  } else {
    // 구형 브라우저
...
const inputWithoutSpaceLength = computed(
  () => inputText.value.replaceAll(" ", "").length
);
const inputWithoutSpaceBytes = computed(() =>
  inputText.value
    .replaceAll(" ", "")
    .split("")
    .reduce((acc, cur) => acc + (cur.charCodeAt(0) > 127 ? 2 : 1), 0)
);
const clear = computed(() => (inputText.value = ""));
</script>
<template>
  <div class="saramin">
    <h1>글자수세기</h1>
    <div class="box">
      ...
      <div class="str-info">
        ...
        <p>
          공백 제외 <span>{{ inputWithoutSpaceLength }}</span> 자 |
          <span>{{ inputWithoutSpaceBytes }}</span> byte
        </p>
      </div>
      <div class="btn-area">
