...
export default {
  ...
  computed: {
    ...
    // 공백 포함 바이트수
    inputByte() {
      return this.inputText.split("").reduce((acc, cur) => {
        return acc + (cur.charCodeAt(0) > 127 ? 2 : 1);
      }, 0);
    },
    // 공백 제외 글자수
    ...
    copyText() {
      if (navigator.clipboard) {
        navigator.clipboard
          ...
          .catch((err) => {
            console.error("복사에 실패했습니다.", err.message);
          });
      } else {
        // 구형 브라우저
    ...
    <div class="box">
      ...
      <div class="str-info">
        <p>
          ...
          <span>{{ inputByte }}</span> byte
        </p>
        <p>
          공백 제외 <span>{{ inputLengthWithoutSpace }}</span> 자 |
          <span>{{ inputByteWithoutSpace }}</span> byte
        </p>
      </div>
      <div class="btn-area">
