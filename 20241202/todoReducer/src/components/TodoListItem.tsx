...
import { checkedReducer } from "./reducer/reducer";

export default function TodoListItem({ todo, handleDelete }: TodoItemProps) {
  const [checked, checkedDispatch] = useReducer(checkedReducer, false);
  ...
  return (
    <li className="flex items-center justify-between p-3 border-b hover:bg-gray-100 transition-colors">
      <div className="flex items-center">
        <input
          checked={checked}
          onChange={() => checkedDispatch({ type: "CHECK", payload: checked })}
          type="checkbox"
